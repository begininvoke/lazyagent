...
- It doesn't talk to any LLM. The only outbound network calls lazyagent makes are explicit `lazyagent limits` checks for Claude, Codex, Grok, Kimi, and Cursor billing/rate-limit data. Everything else (monitoring, prune, compact, and search) is purely local.
