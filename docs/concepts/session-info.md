...
lazyagent computes a single `Session` record per conversation, regardless of which agent produced it. The record is shown in the detail panel (TUI and GUI), returned by `/api/sessions/{id}`, and reused by session-oriented commands such as `lazyagent-cli sessions`, `prune`, `compact`, and `search`.
