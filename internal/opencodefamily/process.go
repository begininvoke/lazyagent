// Package opencodefamily parses the OpenCode-compatible SQLite schema used by
// OpenCode and Kilo.
//
// Compatibility contract:
//   - The database has `session`, `message`, and `part` tables.
//   - `session` exposes id/project_id/parent_id/directory/title/version and
//     time_* columns.
//   - `message.data` and `part.data` carry OpenCode-shaped JSON payloads.
//
// This is intentionally a narrow compatibility layer, not a generic SQLite
// transcript reader. If either upstream changes table names, timestamp units,
// JSON fields, or tool state payloads, update the source-specific fixture tests
// before relaxing the parser.
...
// Source describes an OpenCode-compatible SQLite session store.
type Source struct {
	Agent      string
	EnvVar     string
	DataSubdir string
...
// SessionCache caches OpenCode-compatible sessions keyed by session ID.
...
// NewSessionCache creates an empty OpenCode-compatible session cache.
func NewSessionCache() *SessionCache {
	return &SessionCache{entries: make(map[string]cacheEntry)}
}

// DataDirFor returns the path to an OpenCode-compatible data directory.
func DataDirFor(source Source) string {
	source = normalizeSource(source)
	if d := os.Getenv(source.EnvVar); d != "" {
		return d
	}
	...
	return filepath.Join(home, ".local", "share", source.DataSubdir)
}

// DBPathFor returns the path to an OpenCode-compatible SQLite database.
func DBPathFor(source Source) string {
	source = normalizeSource(source)
	d := DataDirFor(source)
	if d == "" {
		return ""
	}
	return filepath.Join(d, source.DBFile)
}
...
func DiscoverSessionsForFiltered(source Source, cache *SessionCache, cwdMatch func(string) bool) ([]*model.Session, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("open %s db: %w", source.Agent, err)
	}
	defer db.Close()
	...
	for _, ds := range dbSessions {
		...
		session, err := buildSession(db, ds, source.Agent)
		if err != nil {
			continue
		}
...
func normalizeSource(source Source) Source {
	if source.Agent == "" {
		source.Agent = defaultSource.Agent
	}
	if source.EnvVar == "" {
		source.EnvVar = strings.ToUpper(source.Agent) + "_DATA_DIR"
	}
	if source.DataSubdir == "" {
		source.DataSubdir = source.Agent
	}
	if source.DBFile == "" {
		source.DBFile = source.Agent + ".db"
	}
	return source
}
...
func buildSession(db *sql.DB, ds dbSession, agent string) (*model.Session, error) {
	session := &model.Session{
		SessionID:    ds.ID,
		CWD:          ds.Directory,
		Name:         ds.Title,
